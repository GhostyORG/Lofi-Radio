...
if(args[0])
 ...
 const { channel } = message.member.voice;
    ...
    .setLabel(`Pause`)
    .setEmoji(`954003216855597096`)
    .setID(`pause`)
    .setStyle("grey");

    let button2 = new MessageButton()
    .setLabel(`Resume`)
    .setEmoji(`954003217027571742`)
    .setID(`resume`)
    .setStyle("grey");

    let button3 = new MessageButton()
    .setLabel(`Skip`)
    .setEmoji(`954003217216339989`)
    .setID(`skip`)
    .setStyle("grey");
...
let nhh = new MessageActionRow()
    .addComponents(nb1, nb2, nb3, nb4, nb5);

 const nvchelp = new MessageEmbed()
                    .setColor(color)
                    .setDescription(`${m} **Please join VC to play lofi songs**\n${ba}${d} Developer: [Draxler#6145](https://discord.gg/6ePN9bxXRx)`) 
        ...
        if (!channel) return message.channel.send(nvchelp, nhh);
                ...
                const abab = new MessageEmbed()
                    .setColor(color)
                    .setDescription(`${d} Select the language you want to listen.\n${ba}${dev} Developer: [Draxler#6145](https://discord.gg/6ePN9bxXRx)`)
...
const { channel } = message.member.voice;
      ...
      const embed1 = new MessageEmbed()
                    ...
                    .setColor(color)
                    .setTimestamp()
                    .setDescription(`${m} Successfully joined and bound to ${  message.guild.me.voice.channel || message.member.voice.channel}\n${ba}${d} **Playing/Added/Added Bengali Lofi Songs.**`)
...
const supreme2 = ['bengali lofi songs', 'bengali lofi song sleep/relax', 'bengali lofi song study', 'bengali lofi slowed song', 'bengali lofi song collection', 'aesthetic bengali lofi reverb song'];
  let ben = supreme2[Math.floor(Math.random() * supreme2.length)];
        args = `${ben}`;
        const searchString = args
        const url = args ? args.replace(/<(.+)>/g, '$1') : '';
  
         MESSAGE.edit(embed2, nhh)
             b.reply.send(`${p} Playing/Added  Bengali lo-fi songs`, true);
...
const supreme3 = ['hindi lofi songs', 'hindi lofi song sleep/relax', 'hindi lofi song study', 'hindi lofi slowed song', 'hindi lofi song collection', 'aesthetic hindi lofi reverb song', 'romantic lofi hindi song'];
  let hin = supreme3[Math.floor(Math.random() * supreme3.length)];  
...
const supreme4 = ['spanish lofi songs', 'spanish lofi song sleep/relax', 'spanish lofi song study', 'spanish lofi slowed song', 'spanish lofi song collection', 'aesthetic spanish lofi reverb song'];
  let spa = supreme4[Math.floor(Math.random() * supreme4.length)];
        args = `${spa}`;
        const searchString = args
        const url = args ? args.replace(/<(.+)>/g, '$1') : '';
  ...
  const dispatcher = queue.connection.play(ytdl(song.url, { highWaterMark: 1 << 20, quality: "highestaudio" }))
         ...
         if(b.id == "nepali") {
          ...
          if (!channel) return await b.reply.send('You need to be in voice channel', true); 
  const supreme5 = ['nepali lofi songs', 'nepali lofi song sleep/relax', 'nepali lofi song study', 'nepali lofi slowed song', 'nepali lofi song collection', 'aesthetic nepali lofi reverb song'];
  let nep = supreme5[Math.floor(Math.random() * supreme5.length)];

        args = `${nep}`;
        const searchString = args
        const url = args ? args.replace(/<(.+)>/g, '$1') : '';
  
         MESSAGE.edit(embed5, nhh)
